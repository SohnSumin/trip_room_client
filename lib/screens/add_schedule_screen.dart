...
class _AddScheduleScreenState extends State<AddScheduleScreen> {
  ...
  TimeOfDay? _endTime;
  String _selectedColor = 'blue'; // Default color
    ...
    try {
      ...
      if (response.statusCode == 200) {
        if (context.mounted) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(const SnackBar(content: Text('일정이 성공적으로 추가되었습니다!')));
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(SnackBar(content: Text('추가 실패: $errorMessage')));
        }
      }
    } catch (e) {
      if (context.mounted) {
